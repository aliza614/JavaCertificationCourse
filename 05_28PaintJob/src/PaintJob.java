public class PaintJob {
    ...
    {
        if (width*height<=0||areaPerBucket<=0) return -1;
        return (int) Math.round( width*height/areaPerBucket-extraBuckets);
    }*/
        public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
            if (width * height <= 0 || areaPerBucket <= 0 || extraBuckets < 0) return -1;
